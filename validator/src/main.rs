...
pub fn main() {
    ...
    let pbs_config = PbsConfig {
        pbs_url: if matches.is_present("pbs_url") {
            value_of(&matches, "pbs_url").expect("couldn't parse pbs_url")
        } else {
            "".to_string()
        },
        uuid: if matches.is_present("pbs_uuid") {
            value_of(&matches, "pbs_uuid").expect("couldn't parse pbs_uuid")
        } else {
            "".to_string()
        },
    };
    ...
    let mut validator_config = ValidatorConfig {
        ...
        pbs_config: Arc::new(Mutex::new(pbs_config)),
